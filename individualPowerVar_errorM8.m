...
snr=10.^(SNR/10);
        ...
        for tt=1:N
            ...
            while (rmax-rmin>0.0001)
                ...
                lefttrace= trace(  t*(Qx+Phi)*hre8*hre8'-Qx * hrb8 * hrb8'    );
           ...
           % 接着求误差e的对偶变量lambda
            ...
            U21= hre8' * Qx;
            U22 = hre8' * Qx * hre8 -lambda * er(k)  -eta;
            %                                            U_unified = [ -Qx + lambda * eye(M(j))  Qx * hre8; hre8.' * Qx hre8' * Qx * hre8 -lambda * er(k)  -eta  ];
            U_unified =[U11 U12; U21 U22];
            U_unified >= 0;
            %                                         -trace( real( Qx * hre8*hre8.' ) )   -trace( real( Psi * hre8*hre8.' ) ) - lambda * er(k)>=eta;
            ...
            maximize trace(Qz * hre8 * hre8') - psi2 - mu2 * er(k)
            %             minimize kk * er(k)-trace( (Qz - Psi) * hre8 * hre8' )
            subject to
            U11 = mu2 * eye( M(j) ) + Qz;
            U12 = Qz * hre8;
            U21 =  hre8' * Qz;
        ...
        LinRobustM8 (j,k) =sum(Rs) / N;
        %         rsn1(j,k)=sum(Rs1)/N;
        %         rsn2(j,k)=sum(Rs2)/N;
        %         rsn3(j,k)=sum(Rs3)/N;
    end
end
individualM8Var_error=LinRobustM8; 
save individualM8Var_error % 不同的varepsilon产生不同的
