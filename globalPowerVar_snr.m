...
% snr=10.^(SNR/10);
        ...
        for tt=1:N
            ...
            while (rmax-rmin>0.0001)
                ...
                %             1+2*Pa*norm(hae)^2+tau*er+trace(  (Qx+Phi)*hre*hre'   ) <=t *( 1+2*Pa*norm(hab)^2 + trace(Qx * hrb * hrb') );
                % trace(  (Qx+Phi)*hre*hre'-t *( Qx * hrb * hrb' )   ) <=t-1+2*Pa*norm(hae)^2+tau*er;
            ...
            globalM4Var_snrTime=toc(t_start);            
            save globalM4Var_snrTime
           % 接着求误差e的对偶变量lambda
